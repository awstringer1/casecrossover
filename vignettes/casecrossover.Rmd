...
```{r fit-cc-1,cache = TRUE,eval = FALSE}
...
The interface can handle multiple smooth covariates in the obvious way, by including multiple `s()` terms in the formula. Marginal posterior
summaries for each smoothing parameter are returned.

The interface can also handle models in which you include the same covariate as a polynomial, and smooth term. In this case,
the correct marginal means and variances of the linear combinations that form each covariate effect are computed. Try the following (not run):

```{r cc-5,eval = FALSE}
# Add more constraints. Pick the values from the unique values of sim1data$exposure_binned
smoothcontrol2 <- cc_control(
  smooth_prior = list(pc_prior(3,.75)),
  linear_constraints = create_linear_constraints(u = sim1data$exposure_binned,
                                                 whichzero = c(0.5,0.52041),
                                                 nm = "exposure_binned"),
  thetaaccuracy = 50,
  sparsetheta = FALSE, 
  thetarange = c(-10,10)) 

simcc2 <- casecrossover::casecrossover(
  case ~ exposure_binned + s(exposure_binned) + strata(subject),
  data = sim1data,
  control = smoothcontrol,
  verbose = TRUE
)
...
# Or use a polynomial. Note that orthogonal polynomials are NOT supported at this time, raw
# are used regardless of which you specify in poly().
smoothcontrol3 <- cc_control(
  smooth_prior = list(pc_prior(3,.75)),
  linear_constraints = create_linear_constraints(u = sim1data$exposure_binned,
                                                 whichzero = c(0.47959,0.5,0.52041),
                                                 nm = "exposure_binned"),
  thetaaccuracy = 50,
  sparsetheta = FALSE, 
  thetarange = c(-10,10)) 

simcc3 <- casecrossover::casecrossover(
  case ~ poly(exposure_binned,2) + s(exposure_binned) + strata(subject),
  data = sim1data,
  control = smoothcontrol,
  verbose = TRUE
)
